...
async def download_audio_from_url(url):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # after postprocess, it should be mp3
...
async def message_handle(update: Update, context: CallbackContext):
    ...
    if update.message.reply_to_message and update.message.reply_to_message.from_user.id == context.bot.id:
        ...
        if transcription:
            ...
            await update.message.reply_text(f"🤖: <i>{response}</i>", parse_mode=ParseMode.HTML)
            return

    if is_url(text):
        # treat as video URL
        await update.message.reply_text("Downloading and transcribing video from URL... Please wait.")
        ...
        if audio_buf:
            ...
            for message_chunk in split_text_into_chunks(transcribed_text, 4096):
                try:
                    ...
                    await context.bot.send_message(update.effective_chat.id, text_reply, parse_mode=ParseMode.HTML)
                except telegram.error.BadRequest:
                    await context.bot.send_message(update.effective_chat.id, message_chunk)
        else:
            await update.message.reply_text("Failed to download or extract audio from URL.")
    else:
...
def run_bot() -> None:
    ...
    application.add_handler(CommandHandler("set_language", set_language_handle, filters=user_filter))
    application.add_handler(CommandHandler("set_model", set_model_handle, filters=user_filter))
